...
/** Remove an item from the aquarium
...
/**
 * Determines if fish is over carp
 * \param item The fish that is being eaten
 * \returns true if collision, false otherwise
 */
bool CAquarium::IsOver(std::shared_ptr<CItem> item)
{
...
void CAquarium::XmlItem(const std::shared_ptr<xmlnode::CXmlNode> &node)
{
	...
	if (type == L"angelfish")
	{
		item = make_shared<CFishAngelfish>(this);
	}
	if (type == L"carp")
	{
	...
	if (type == L"scull")
	{
		item = make_shared<CDecorSkull>(this);
	}
